...
class MMDLoader extends Loader {
	 ...
	 */
	load( url, onLoad, onProgress, onError, CUSTOM_ ) {
		...
		if(url.indexOf("blob:") !== -1){ modelExtension = "pmx" }
		...
		this[ modelExtension === 'pmd' ? 'loadPMD' : 'loadPMX' ]( url, function ( data ) {
			...
			onLoad(	builder.build( data, resourcePath, onProgress, onError, CUSTOM_ )	);
	 ...
	 * @param {function} onError
	 */
	loadPMX( url, onLoad, onProgress, onError, CUSTOM_ ) {
...
class MeshBuilder {
	 ...
	 * @return {SkinnedMesh}
	 */
	build( data, resourcePath, onProgress, onError, CUSTOM_ ) {

		const geometry = this.geometryBuilder.build( data );
		const material = this.materialBuilder
			.setCrossOrigin( this.crossOrigin )
			.setResourcePath( resourcePath )
			.build( data, geometry, onProgress, onError, CUSTOM_ );
...
class MaterialBuilder {
		...
		for ( let i = 0; i < data.metadata.materialCount; i ++ ) {
			...
			if ( data.metadata.format === 'pmd' ) {
				...
				if ( material.textureIndex !== - 1 ) {
					...
					this.cs2 = data; if(CUSTOM_) {CS_changeTextureToAnotherUrl(CUSTOM_, data, material.textureIndex, this.CustomFile)};
				...
				if ( material.envTextureIndex !== - 1 && ( material.envFlag === 1 || material.envFlag == 2 ) ) {
					if(CUSTOM_) {CS_changeTextureToAnotherUrl(CUSTOM_, data, material.textureIndex, this.CustomFile) };
					params.matcap = this._loadTexture(
	...
	_loadTexture( filePath, textures, params, onProgress, onError ) {
		...
		var cus; if(this.CustomFile){if(this.CustomFile.indexOf(filePath) !== -1){cus = "tga"} }
		if ( loader === null ) {

			loader = ( filePath.slice( - 4 ).toLowerCase() === '.tga' ) || cus == "tga"
				? this._getTGALoader()
				: this.textureLoader;
...
//Custom
function CS_changeTextureToAnotherUrl(obj, data, i, file){
	const e = data.textures[i];
	for (let k = 0; k < data.textures.length; k++) {
		const tex = data.textures[k];
		for (let s = 0; s < obj.length; s++) {
			const q = obj[s];
			if (tex.indexOf("\\") !== -1) {
				let adress = tex.split("\\");
				let path = adress.length - 1;
				let co = adress.join("/");
				let name = getName(obj[s].name, path);
				
				if (co == name || co == q.name) {
					var lastId = q.url.split("/");
					var le = lastId.length - 1;
					data.textures[k] = lastId[le];
					if(q.name.endsWith(".tga") || name.endsWith(".tga")){
						file.push(lastId[le]);
					}
				}
			} else {
				if (tex == obj[s].name) {
					...
					var le = lastId.length - 1;
					data.textures[k] = lastId[le];
					if(q.name.endsWith(".tga")){
						file.push(lastId[le]);
					}
				} else {
					let name = getName(obj[s].name, 1);
				
					if (tex == name || tex == q.name) {
						var lastId = q.url.split("/");
						var le = lastId.length - 1;
						data.textures[k] = lastId[le];
						if(q.name.endsWith(".tga") || name.endsWith(".tga")){
